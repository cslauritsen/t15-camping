...
pub(crate) async fn get_root(req: Request<State>) -> tide::Result<impl Into<Response>> {
    Ok("Hello, world!")
}

/// List all databases
pub(crate) async fn list_dbs(req: Request<State>) -> tide::Result<impl Into<Response>> {
    let client = req.state().mongo();
